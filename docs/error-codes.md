...
| `TOMIX_RENAME_BREAKS_REFS` | 1 | `--strict-refs` was set and the rename (`set -q name`, `mv`) would leave DAX expressions referencing the old name. By default renames rewrite referencing DAX automatically, so this only fires for references that cannot be rewritten (role RLS filters, or a calendar that shares a table's name) — or, under `--no-fix-refs`, for any reference. Without `--strict-refs` the rename proceeds with a warning listing the objects left broken. |
