...
public sealed class MacroHandler
{
    ...
    public MdlResult<MacroFileResult> Init(string? explicitPath, bool force)
    {
        var path = ResolvePath(explicitPath);
        if (File.Exists(path) && !force)
            return MdlResult<MacroFileResult>.Fail(
                "MDL_MACRO_FILE_EXISTS",
                $"Macros file already exists: {path}. Use --force to overwrite.",
                exitCode: 1);

        Save(path, new MacroDocument([]));
        return MdlResult<MacroFileResult>.Ok(new MacroFileResult(path));
    }

    public MdlResult<MacroListResult> List(string? explicitPath)
    {
        var path = ResolvePath(explicitPath);
        if (!File.Exists(path))
            return MdlResult<MacroListResult>.Ok(new MacroListResult(path, []));

        var document = Load(path);
        return MdlResult<MacroListResult>.Ok(new MacroListResult(path, Project(document.Actions)));
    }

    public MdlResult<MacroSavedResult> Add(MacroAddRequest request)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.ScriptPath))
        {
            if (!File.Exists(request.ScriptPath))
                return MdlResult<MacroSavedResult>.Fail(
                    "MDL_MACRO_SCRIPT_NOT_FOUND",
        ...
        Save(path, document);
        return MdlResult<MacroSavedResult>.Ok(new MacroSavedResult(path, Project(action), "added"));
    }

    public MdlResult<MacroSavedResult> Set(string? explicitPath, string nameOrId, string property, string value)
    {
        var path = ResolvePath(explicitPath);
        if (!File.Exists(path))
        ...
        Save(path, document);
        return MdlResult<MacroSavedResult>.Ok(new MacroSavedResult(path, Project(updated), "updated"));
    }

    public MdlResult<MacroSavedResult> Remove(string? explicitPath, string nameOrId)
    {
        var path = ResolvePath(explicitPath);
        if (!File.Exists(path))
            return MissingFile<MacroSavedResult>(path);
        ...
        Save(path, document);
        return MdlResult<MacroSavedResult>.Ok(new MacroSavedResult(path, Project(action), "removed"));
    }

    public MdlResult<MacroSortResult> Sort(string? explicitPath)
    {
        var path = ResolvePath(explicitPath);
        if (!File.Exists(path))
        ...
        Save(path, document);
        return MdlResult<MacroSortResult>.Ok(new MacroSortResult(path, sorted.Count));
    }

    private static MdlResult<T> MissingFile<T>(string path)
        => MdlResult<T>.Fail(
            "MDL_MACRO_FILE_NOT_FOUND",
            $"Macros file not found: {path}. Run 'mdl macro init' to create one, or pass --macros <path>.",
            exitCode: 1);

    private static MdlResult<T> MissingMacro<T>(string nameOrId)
        => MdlResult<T>.Fail(
            "MDL_MACRO_NOT_FOUND",
    ...
    private static string ResolvePath(string? explicitPath)
    {
        ...
        var envPath = Environment.GetEnvironmentVariable("MDL_MACROS_PATH");
        ...
        return Path.Combine(MdlPaths.ConfigDirectory, "macros.json");
    }
