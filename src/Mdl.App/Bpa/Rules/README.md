...
`bpa-rules.json` is the default Best Practice Analyzer rule set used by `mdl bpa` when no other
ruleset is selected.
...
`BpaRuleLoader` can also fetch Microsoft's canonical set on demand (e.g. `mdl bpa run --ruleset
