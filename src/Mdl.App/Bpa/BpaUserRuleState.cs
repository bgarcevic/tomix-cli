...
/// <c>{config}/bpa-disabled.json</c> (<c>~/.mdl</c> or <c>$MDL_CONFIG_DIR</c>). A disabled rule is
...
public sealed class BpaUserRuleState
{
    ...
    /// <param name="configDirectory">Override the config directory (used by tests); defaults to <see cref="MdlPaths.ConfigDirectory"/>.</param>
    public BpaUserRuleState(string? configDirectory = null)
        => _path = Path.Combine(configDirectory ?? MdlPaths.ConfigDirectory, FileName);
