...
- `CliConnectionState.ReportName`/`ReportPortFile` cache the Desktop report name for display. `ConnectHandler.Show` revalidates it with `PowerBiDesktopDiscovery.StillServes` (does that port file still hold this session's port?) and clears both fields when it fails, so no caller — text or JSON — can render a stale name. The cache exists because the alternative, re-reading the window title per invocation, costs ~220ms; validating costs two small file operations. Ports change on every Desktop restart, so the stale path is the common one.
