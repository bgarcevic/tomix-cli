...
internal sealed class ProfileCommand : ICommandModule
{
    ...
    public ProfileCommand(CliStateStore state) => _state = state;
    ...
    private Command BuildShow()
    {
        ...
        {
            var format = GlobalOptions.OutputFormatValue(parseResult);
            if (!CommandOutput.TryValidateFormat(parseResult, format, "profile show", OutputFormats.Text, OutputFormats.Json))
                return 2;
    ...
    private Command BuildRemove()
    {
        ...
        {
            var format = GlobalOptions.OutputFormatValue(parseResult);
            if (!CommandOutput.TryValidateFormat(parseResult, format, "profile remove", OutputFormats.Text, OutputFormats.Json))
                return 2;
    ...
    private Command BuildSet()
    {
        ...
        command.SetAction(parseResult =>
        {
            ...
            var result = new ProfileHandler(_state).Set(new ProfileSetRequest(
